import { Chain, OneOf } from '@octas/types';
import {
  ConnectorEventMap,
  CreateConnectorFn,
} from './connectors/createConnector';
import { Compute, ExactPartial, RemoveUndefined } from './types/utils';
import {
  createStorage,
  getDefaultStorage,
  type Storage,
...
import { createEmitter, Emitter, EventData } from './createEmitter';
import { uid } from '@octas/utils';
import { OctaClient } from './client';
import { AbiTable } from '@octas/abi';
import { persist, subscribeWithSelector } from 'zustand/middleware';
import { ChainNotConfiguredError } from '@octas/errors';
import { Client, ClientParams, createClient } from './createClient';
...
>(
  parameters: CreateConfigParameters<chains, octaClient, connectorFns>
): Config<chains, octaClient, connectorFns> {
  const {
  ...
  const clients = new Map<number, Client<octaClient, chains[number]>>();
  function getClient<chainId extends chains[number]['id']>(
    config: { chainId?: chainId | chains[number]['id'] | undefined } = {}
  ): Client<octaClient, Extract<chains[number], { id: chainId }>> {
    ...
    type Return = Client<octaClient, Extract<chains[number], { id: chainId }>>;
    {
  ...
  if (syncConnectedChain)
    store.subscribe(
      ({ connections, current }) =>
        current ? connections.get(current)?.connector.chain.id : undefined,
      (chainId) => {
  ...
  return {
    ...
    _internal: {
      ...
      ssr: Boolean(ssr),
      syncConnectedChain,
      octaClient: rest.octaClient as octaClient,
      chains: {
        setState(value) {
          const nextChains = (
...
> = Compute<
  {
    ...
    | ({ octaClient: octaClient } & {
        [key in keyof ClientParams]?:
          | ClientParams[key]
          | { [_ in chains[number]['id']]?: ClientParams[key] | undefined }
          | undefined;
      })
    | {
        client(parameters: {
          chain: chains[number];
        }): Client<octaClient, chains[number]>;
      }
  >
>;
...
> = {
   ...
   * @internal
   */
  _internal: Internal<octaClient>;
};

type Internal<octaClient extends OctaClient<AbiTable> = OctaClient<AbiTable>> =
  {
    revalidate: () => Promise<void>;
    readonly store: Mutate<StoreApi<any>, [['zustand/persist', any]]>;
    readonly ssr: boolean;
    readonly syncConnectedChain: boolean;
    readonly octaClient: octaClient;
    ...
    connectors: {
      ...
      setState(
        value: Connector[] | ((state: Connector[]) => Connector[])
      ): void;
      subscribe(
